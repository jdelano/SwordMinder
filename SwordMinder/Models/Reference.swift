...
struct Reference: Codable, Equatable, Comparable {
    ...
    private static let verseCounts = [
   ...
   /// Initializes a new `Reference` based on a String representation of a verse's reference
    ...
    static func random(in book: Book, chapter: Int? = nil) -> Reference {
        var reference = Reference(book: book)
        if let chapter, chapter >= 1, chapter <= reference.lastChapter {
            reference.chapter = chapter
        } else {
            reference.chapter = Int.random(in: 1...reference.lastChapter)
        }
        reference.verse = Int.random(in: 1...reference.lastVerse)
        return reference
    }
}
